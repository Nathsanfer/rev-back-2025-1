...
export const getUsers = (req, res) => {
    ...
    if (!users) {
        return res.status(404).send({ mensage: "Não há usuários cadatrados" });
    }
    return res.status(200).send({ message: "Usuários encontrados", user});
};

export const createUser = async (req, res) => {
    const { name, email, password } = req.body;

    const userAlready = usersRepository.getUserByEmail(email);
